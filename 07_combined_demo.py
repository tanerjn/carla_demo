#!/usr/bin/env python3
"""CarlaAir Quick Start - Step 7: Air-Ground Joint Simulation"""
import carla
import airsim
import time
import math

# Connect to Dual APIs
...
print("Dual APIs conneccted successfully!")

# CARLA: Weather + Vehicles
...
for sp in spawns[:8]:
    ...
    if v:
        ...
        vehicles.append(v)
print(f"CARLA: {len(vehicles)} vehicles")

# AirSim: Drone
client_air.takeoffAsync().join()
client_air.moveToPositionAsync(80, 0, -30, 5).join()
print("AirSim: Drone in position")

# Joint operation: Drone flies around the city + Ground Traffic
print("\nRunning joint simulation for 15 seconds...")
for i in range(30):
    t = i / 30 * 2 * math.pi
    ...
    if i % 10 == 0:
        ...
        print(f"  [{i//2}s] Drone: ({pos.x_val:.0f},{pos.y_val:.0f},{pos.z_val:.0f})")
    time.sleep(0.5)
...
print("Joint simulation demo complete!")
